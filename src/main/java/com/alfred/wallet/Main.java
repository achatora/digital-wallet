...
public class Main {
  public static void main(String[] args) {
    ...
    System.out.printf("Deposit success! New wallet balance: %s %.2f%n",
        wallet.getCurrency(),
        wallet.getBalance());

    // Withdrawal
    wallet.withdraw(new BigDecimal("40.00"));
    System.out.printf("Withdrawal success! New wallet balance: %s %.2f%n",
        wallet.getCurrency(),
        wallet.getBalance());
    ...
    System.out.printf("Account Name: %s | New balance: %s%n",
        customer.getName(),
        wallet.getBalance());
    ...
    // Transaction History
    System.out.println(wallet.getTransactionHistory().size());

    for (Transaction transaction : wallet.getTransactionHistory()) {
      System.out.printf("Transaction Type: %s | Amount: %s | Timestamp: %s%n",
          transaction.getType(),
          transaction.getAmount(),
          transaction.getTimestamp());
    }

    for (Transaction transaction : wallet2.getTransactionHistory()) {
      System.out.printf("Transaction Type: %s | Amount: %s | Timestamp: %s%n",
          transaction.getType(),
          transaction.getAmount(),
          transaction.getTimestamp());
